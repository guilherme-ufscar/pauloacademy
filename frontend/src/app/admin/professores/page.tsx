...
export default function ProfessoresPage() {
  ...
  const renderGrid = (list: Professor[]) => (
  ...
  const docentes = professors.filter(p => (p.team_type || 'docente') === 'docente')
  const comerciais = professors.filter(p => p.team_type === 'comercial')
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500">{professors.length} integrantes cadastrados</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-10">
          <div>
            <h2 className="text-lg font-bold text-gray-900 mb-4">Corpo Docente</h2>
            {renderGrid(docentes)}
          </div>
          <div>
            <h2 className="text-lg font-bold text-gray-900 mb-4">Corpo Administrativo Comercial</h2>
            {renderGrid(comerciais)}
          </div>
        </div>
      )}
    </div>
